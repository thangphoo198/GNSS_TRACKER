...
{
	{GPIO_PIN_NO_6, PIN_DIRECTION_OUT, PIN_NO_EDGE, PIN_PULL_DISABLE, PIN_LEVEL_LOW},
	{GPIO_PIN_NO_70, PIN_DIRECTION_OUT, PIN_NO_EDGE, PIN_PULL_DISABLE, PIN_LEVEL_LOW},
...
static void quec_gpio_test(void * argv)
{
	...
	while(1) {
		...
		for(i=0; i<QUEC_ARR_SIZE(__quec_gpio_cfg) ; i++) {
			ql_gpio_set_level(__quec_gpio_cfg[i].gpio_pin_num, led);
			test_log("===Set gpio[%d] level is %d=== \n", __quec_gpio_cfg[i].gpio_pin_num, led);
		}
		led^=1;
		ql_rtos_task_sleep_ms(500);
