...
#define mqtt_exam_log(fmt, args...) usb_log(fmt, ##args)
static void pub_mqtt(char *topic, char *mess);
...
void pub_mqtt(char *topic, char *mess)
{
		...
		usb_log("Return code from MQTT publish is %d", rc);
}
...
static void MQTTEchoTask(void *argv)
{
	...
	connectData.clientID.cstring = "0c45a17d9d4642adaa87820c44667309"; // ID
	...
	while (1)
	{
		...
		char payload[30];
		sprintf(payload, "hello %d", count);
		count++;
		pub_mqtt("EC800M_REC",payload);
